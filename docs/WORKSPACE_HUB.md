...
- **同期手段**: `IMPORTRANGE` / `QUERY(IMPORTRANGE(...))`（**原本 → ハブの片方向**）。相互リンクはしない（ハブでチェックしても原本は変わらない）
- **見た目**: マシンレクチャー原本とハブは同じ緑ヘッダー／列幅／チェック（`setupMachineImport_` が両方に適用）
- **入会者ハブ**: A:E のみ。原本の F/G は残して非表示（値は消さない）。行の追加は IMPORTRANGE で自動追従
...
| `入会者一覧＋自動メール管理` | 同名 | `IMPORTRANGE` A:E（D・E のメール印のみ。F/G の済フラグは原本に残す） |
